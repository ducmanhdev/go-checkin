    ...
    <div class="mb-4 md:flex md:items-center md:justify-between gap-3 shrink-0">
      <h2 class="heading-1 mb-0 flex items-center md:block">
        <img
          ...
          @click="$router.push({name: 'reminder'})"
...
import type {
  GetCampaignReminderDetailResponse,
} from "gap-nodejs-sdk";
...
const data = ref<GetCampaignReminderDetailResponse | null>(null);
const isGetDataLoading = ref(false);
const handleGetData = async () => {
  try {
    isGetDataLoading.value = true;
    data.value = await goCheckInSdk.campaign.getCampaignReminderDetail({
